...
Including the $n-1$ term gives the proper unbiased estimator of a larger population based on a smaller sample.
...
\begin{equation}
   P = \int_{Z} g(z_1,z_2)dz_2 dz_1= \int_{X} g(\frac{x_1-\mu_1}{\sigma_1},\frac{1}{\sqrt{1-\rho^2}}(\frac{x_2-\mu_2}{\sigma_2}-\rho\frac{x_1-\mu_1}{\sigma_1}))\cdot|J_{\vec{f}}|dx_2 dx_1
...
Moving onto the next foundational proposition, it is valuable to examine how the expected value and variance of bivariate data behaves.
...
Now, if husbands are indeed taller than their wives, it should be the case that $X_1-X_2>0$ and $Y>0$ most of the time.
...
We can also see that the Marsh data set has a higher probability of a positive height difference than the Galton one, which may become more relevant soon.
...
To perform a two-tailed test on the gathered data over a 95\% confidence interval, we introduce the following test statistic:
\begin{equation}
...
Noting that for the Galton data, $n=205$ and for the Marsh data, $n=199$, Table~\ref{tab:comparison-of-difference-probabilities} dictates that $z_{Marsh}\approx-1.1745$ and $z_{Galton}\approx-1.6324$.
...
To study the average heights for each set, we introduce here the student-$t$ distribution, which is a generalization of the normal distribution.
It approaches the normal distribution as the sample size increases ($n\rightarrow{\infty}$).
The actual expression is given by the following:
\begin{equation}
...
``Degrees of freedom'' here means the number of values that are allowed to vary; take for the sake of illustration, a class of ten students.
...
Returning to the student-$t$ distribution, the following test statistic will be utilized:
\begin{equation}
t=\frac{\bar{x}_1-\bar{x}_2-(\mu_1-\mu_2)}{\sqrt{\frac{s_1^2}{n_1}+\frac{s_2^2}{n_2}}}
\label{eq:comparing-heights-by-sex-across-datasets-t-test}
\end{equation}
To simplify our calculations, as this distribution can be quite complex to work with, we will approximate degrees of freedom with $\nu=\min\{n_1-1,n_2-1\}$.
The null hypothesis here is $\mu_1=\mu_2$.
...
Attempting once more to focus on the present endeavor, we observe that $\nu=198$ and according to \eqref{eq:comparing-heights-by-sex-across-datasets-t-test}, $t_{males}=-4.1573$ and $t_{females}=-3.9111$ whose magnitudes are greater than 1.97 (which one can find by solving \eqref{eq:t-score-integral} for $t$ with $P=0.025$ and $P=0.975$).
These large t-scores indicate that the groups are significantly different and the null hypothesis must be rejected ($\mu_1\neq \mu_2$).
This conclusion is also supported by the p-score for men ($4.79\times10^{-5}$) and women ($1.26\times10^{-4}$) being very close to zero (if the probability for the t-score is less than 0.05, the null hypothesis is rejected).
...
Earlier in this paper we used \eqref{eq:taller-male-proportion-z-statistic} to determine that there was not a statistically significant difference between the two datasets in the proportion of couples who had a taller man.
Now, we will analyze a similar hypothesis, but one that focuses not on the number of couples with a taller male, but on the magnitude of the height difference.
We will use the following $t$-statistic:
\begin{equation}
   t=\frac{\bar{d}-\mu_d}{\bar{s}/\sqrt{n}}
   \label{eq:difference-t-test}
\end{equation}
where $\bar{d}$ is the mean of the actual differences in heights, $\bar{s}$ is the standard deviation in the actual difference in heights, and $\mu_d$ is the actual difference mean claim.
Our null hypothesis is $H_0: \mu_{d} < 0$, and our alternative hypothesis is $H_a:\mu_{d} \nless 0$.
Since this is a one-tailed test, we want to know if $\bar{d}$ is above the critical $t$-score for 95\% confidence ($t\approx1.65$ for both datasets).
Because of our null hypothesis, we set $\mu_d = 0$.
...
Let us test one final hypothesis: there is no difference between the mean height difference of both datasets ($H_0:\bar{d}_1 = \bar{d}_2$).
...
From this result we can conclude that there is not a significant difference between the actual mean height difference of the two datasets.